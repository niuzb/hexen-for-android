...
void SN_InitSequenceScript(void)
{
	...
	while(true)
	{	
		boolean s = SC_GetString();
		if(s == false){
			LOGI("will be break, %s", sc_String);
			break;
		}
		LOGI("get sc_String %s",sc_String);
		if(*sc_String == ':')
		{
			LOGI("get a :");
			if(inSequence != -1)
			...
			tempDataStart = (int *)Z_Malloc(SS_TEMPBUFFER_SIZE, 
				PU_STATIC, NULL);
			//tempDataStart = (int *)malloc(SS_TEMPBUFFER_SIZE);
			LOGI("get a :%d,tempDataStart %p ", __LINE__, tempDataStart);
			memset(tempDataStart, 0, SS_TEMPBUFFER_SIZE);

			tempDataPtr = tempDataStart;
			
			LOGI("get a :%d,tempDataPtr %p %d", __LINE__, tempDataPtr, *tempDataPtr);

			//tempDataPtr++;
			LOGI("get a :%d, %p ", __LINE__, tempDataPtr);
	...
	LOGI("end of SN_InitSequenceScript %d", __LINE__);
}
