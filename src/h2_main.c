...
{
	"hexen.wad",
	"/sdcard/app-data/tk.niuzb.wolf3d/hexen.wad"
};
#else
static char *wadfiles[MAXWADFILES] =
{
...
void H2_GameLoop(void)
{
	...
	while(1)
	{
		...
		//S_UpdateSounds(players[displayplayer].mo);//niuzb
...
static void DrawAndBlit(void)
{
    ...
    for gl
	    if (use_gl) 
	    {
    	        if(!netgame)
		{
			OGL_DrawPatch(160, viewwindowy+5, W_GetNumForName("PAUSED"));
		}
		else
		{
			OGL_DrawPatch(160, 70, W_GetNumGorName("PAUSED"));
		}
	    }
	    else 
#endif
...
static void PageDrawer(void)
{
   ...
   {
    for gl
    OGL_DrawRawScreen(W_GetNumForName(pagename));
    if (demosequence == 1) 
    {
		OGL_DrawPatch(4, 160, W_GetNumForName("ADVISOR");     
    	        UpdateState |= I_FULLSCRN;
    }
   }
   else 
...
static void CreateSavePath(void)
{
	static char creationPath[255];
	int len;
	char *homedir;

	if(cdrom == true)
	{
		SavePath = "/cdrom/hexndata";
	}
	len = strlen(SavePath);
	if (len >= 120) I_Error("Save path too long\n");
	strcpy(creationPath, SavePath);

/* XXX Create a save path in home directory */
#ifndef MUSTDIE
	homedir=getenv("HOME");
	if (!homedir) I_Error("Cannot locate home directory!\n");
	strcpy(creationPath,homedir);
	strcat(creationPath,"/.hexendata/");
#else
	strcpy(creationPath,"HEXNDATA\\");
#endif
	mkdir(creationPath,0700);
	SavePath=creationPath;
}
