...
void SV_SaveGame(int slot, char *description)
{
	...
	StreamOutLong(gamemap);
	StreamOutLong(gameskill);
...
void SV_LoadGame(int slot)
{
	...
	LOGI("%s, %d, %p",__FUNCTION__, __LINE__,SavePtr.b);
...
static void ArchiveWorld(void)
{
	...
	for(i = 0, li = lines; i < numlines; i++, li++)
	{
		...
		StreamOutLong(li->special);
		StreamOutLong(li->arg1);
		StreamOutLong(li->arg2);
		StreamOutLong(li->arg3);
		StreamOutLong(li->arg4);
		StreamOutLong(li->arg5);
		for(j = 0; j < 2; j++)
		{
			if(li->sidenum[j] == -1)
			{
