...
static void SCDefineKey(int option);
static void SCAlwaysRun(int option);
static void SCUseMouse(int option);
static void SCMButFire(int option);
static void SCMButStrafe(int option);
static void SCMButForward(int option);
static void SCMButJump(int option);
static void SCTurboMouse(int option);
static void DrawMainMenu(void);
static void DrawClassMenu(void);
static void DrawSkillMenu(void);
static void DrawOptionsMenu(void);
static void DrawControlsMenu(void);
static void DrawControlsMenu1(void);
static void DrawControlsMenu2(void);
static void DrawControlsMenu3(void);
static void DrawControlsMenu4(void);
static void DrawControlsMenuM(void);
...
extern int mousebstrafe, mousebfire, mousebforward, mousebjump;
extern boolean gamekeydown[256]; // The NUMKEYS macro is local to g_game
extern int usemouse;
extern int speed_lock;
...
static int DefineKeyOption=0;
static int *CurrentKeyDefinition;
boolean askforquit;
boolean typeofask;
...
static int *KeyPointers[]= {
 &key_up, &key_down, &key_right, &key_left, &key_straferight, &key_strafeleft,
 &key_strafe, &key_jump, &key_speed, &key_use, &key_fire, &key_useartifact,
 &key_flyup, &key_flydown, &key_flycenter, &key_lookup, &key_lookdown, &key_lookcenter,
 &key_invleft,&key_invright 
};
...
static MenuItem_t OptionsItems[] =
{
	...
	{ ITT_EFUNC, "ALWAYS RUN :", SCAlwaysRun, 0, MENU_CONTROLS },
	{ ITT_SETMENU, "CONFIGURE CONTROLS", NULL, 0, MENU_CONTROLS },
...
{
	{ ITT_SETMENU, "DEFINE KEYS", NULL, 0, MENU_CONTROLS1 },
	{ ITT_SETMENU, "CONFIGURE MOUSE", NULL, 0, MENU_CONTROLSM },
};

static MenuItem_t ControlsItems1[] =
{
	{ ITT_EFUNC, "UP:", SCDefineKey, 2, MENU_NONE },
	{ ITT_EFUNC, "DOWN:", SCDefineKey, 3, MENU_NONE },
	{ ITT_EFUNC, "RIGHT:", SCDefineKey, 4, MENU_NONE },
	{ ITT_EFUNC, "LEFT:", SCDefineKey, 5, MENU_NONE },
	{ ITT_EFUNC, "STRAFE RIGHT:", SCDefineKey, 6, MENU_NONE },
	{ ITT_EFUNC, "STRAFE LEFT:", SCDefineKey, 7, MENU_NONE },
	{ ITT_SETMENU, "MORE...", SCDefineKey, 0, MENU_CONTROLS2 }
};

static MenuItem_t ControlsItems2[] =
{
	{ ITT_EFUNC, "STRAFE:", SCDefineKey, 8, MENU_NONE },
	{ ITT_EFUNC, "JUMP:", SCDefineKey, 9, MENU_NONE },
	{ ITT_EFUNC, "RUN:", SCDefineKey, 10, MENU_NONE },
	{ ITT_EFUNC, "USE:", SCDefineKey, 11, MENU_NONE },
	{ ITT_EFUNC, "FIRE:", SCDefineKey, 12, MENU_NONE },
	{ ITT_EFUNC, "USE ARTIFACT:", SCDefineKey, 13, MENU_NONE },
	{ ITT_SETMENU, "MORE...", SCDefineKey, 0, MENU_CONTROLS3 }
};

static MenuItem_t ControlsItems3[] =
{
	{ ITT_EFUNC, "FLY UP:", SCDefineKey, 14, MENU_NONE },
	{ ITT_EFUNC, "FLY DOWN:", SCDefineKey, 15, MENU_NONE },
	{ ITT_EFUNC, "FLY CENTER:", SCDefineKey, 16, MENU_NONE },
	{ ITT_EFUNC, "LOOK UP:", SCDefineKey, 17, MENU_NONE },
	{ ITT_EFUNC, "LOOK DOWN:", SCDefineKey, 18, MENU_NONE },
	{ ITT_EFUNC, "LOOK CENTER:", SCDefineKey, 19, MENU_NONE },
	{ ITT_SETMENU, "MORE...", SCDefineKey, 0, MENU_CONTROLS4 }
};

static MenuItem_t ControlsItems4[] =
{
	{ ITT_EFUNC, "INV LEFT:", SCDefineKey, 20, MENU_NONE },
	{ ITT_EFUNC, "INV RIGHT:", SCDefineKey, 21, MENU_NONE },
};


static MenuItem_t ControlsItemsM[] =
{
	{ ITT_EFUNC, "USE MOUSE:", SCUseMouse, 0, MENU_NONE },
	{ ITT_LRFUNC, "FIRE BUTTON:", SCMButFire, 0, MENU_NONE },
	{ ITT_LRFUNC, "STRAFE BUTTON:", SCMButStrafe, 0, MENU_NONE },
	{ ITT_LRFUNC, "FORWARD BUTTON:", SCMButForward, 0, MENU_NONE },
	{ ITT_LRFUNC, "JUMP BUTTON:", SCMButJump, 0, MENU_NONE },
	{ ITT_LRFUNC, "TURBO MOUSE:", SCTurboMouse, 0, MENU_NONE },
	{ ITT_EMPTY, NULL, NULL, 0, MENU_NONE },
};
...
static void DrawOptionsMenu(void)
{
	MN_DrTextB(messageson ? "ON" : "OFF" , 196, 30);
	MN_DrTextB(speed_lock ? "ON" : "OFF" , 210, 90);
	DrawSlider(&OptionsMenu, 3, 10, mouseSensitivity);
}

char *MN_GetKeyName(int);

//---------------------------------------------------------------------------
//
// PROC DrawControlsMenu
//
//---------------------------------------------------------------------------
...
static void DrawControlsMenu1(void)
{
  int i;
  for (i=0; i<6; i++)
   MN_DrTextB((ControlsItems1[i].option==DefineKeyOption ? "PRESS A KEY": MN_GetKeyName(*KeyPointers[ControlsItems1[i].option-2])),
              170,i*ITEM_HEIGHT+5);
}
static void DrawControlsMenu2(void)
{
  int i;
  for (i=0; i<6; i++)
   MN_DrTextB((ControlsItems2[i].option==DefineKeyOption ? "PRESS A KEY": MN_GetKeyName(*KeyPointers[ControlsItems2[i].option-2])),
              170,i*ITEM_HEIGHT+5);
}
static void DrawControlsMenu3(void)
{
  int i;
  for (i=0; i<6; i++)
   MN_DrTextB((ControlsItems3[i].option==DefineKeyOption ? "PRESS A KEY": MN_GetKeyName(*KeyPointers[ControlsItems3[i].option-2])),
              170,i*ITEM_HEIGHT+5);
}
static void DrawControlsMenu4(void)
{
  int i;
  for (i=0; i<2; i++)
   MN_DrTextB((ControlsItems4[i].option==DefineKeyOption ? "PRESS A KEY": MN_GetKeyName(*KeyPointers[ControlsItems4[i].option-2])),
              170,i*ITEM_HEIGHT+5);
}
static void DrawControlsMenuM(void)
{
  MN_DrTextB(usemouse ? "ON":"OFF",180,5);
  MN_DrTextB(MButtons[mousebfire+1],180,25);
  MN_DrTextB(MButtons[mousebstrafe+1],180,45);
  MN_DrTextB(MButtons[mousebforward+1],180,65);
  MN_DrTextB(MButtons[mousebjump+1],180,85);
  if (mouse_scale_factor>25) mouse_scale_factor=25;
  DrawSlider(&ControlsMenuM, 6, 25, mouse_scale_factor-1);
}
...
boolean MN_Responder(event_t *event)
{
	...
	if (DefineKeyOption) {
	 if (event->type==ev_keydown) {
	  for (i=0; i<sizeof(KeyPointers)/sizeof(*KeyPointers); i++) {
	   if (*(KeyPointers[i])==event->data1) *(KeyPointers[i])=0;
	  }
 	  *CurrentKeyDefinition=event->data1;
  	  DefineKeyOption=0;
	  S_StartSound(NULL, SFX_DOOR_LIGHT_CLOSE);
	  return(true);
	 }
...
static void SCDefineKey(int option) {
  CurrentKeyDefinition=KeyPointers[option-2];
  DefineKeyOption=option;
  S_StartSound(NULL, SFX_DOOR_LIGHT_CLOSE);
}
...
static void SCMChButt(int *b, int o) {
 switch(o) {
  case RIGHT_DIR:
   if (++(*b)>2) *b=-1; 
   break;
...
static void SCMButFire(int option) {
 SCMChButt(&mousebfire,option);
}

static void SCMButStrafe(int option) {
 SCMChButt(&mousebstrafe,option);
}

static void SCMButForward(int option) {
 SCMChButt(&mousebforward,option);
}

static void SCMButJump(int option) {
 SCMChButt(&mousebjump,option);
}

static void SCTurboMouse(int option) {
 switch (option) {
  case LEFT_DIR:
   if (--mouse_scale_factor<1) mouse_scale_factor=1;
   break;
  case RIGHT_DIR:
   if (++mouse_scale_factor>25) mouse_scale_factor=25;
  break;
