...
int key_fire, key_use, key_strafe, key_speed, speed_lock;
...
//printf ("cons: %i\n",cmd->consistancy);
	...
	speed = gamekeydown[key_speed] || speed_lock || joybuttons[joybspeed];
#ifdef __WATCOMC__
	if(useexterndriver)
	{
...
//
	...
	if (gamekeydown[key_up])
	{
		...
		//LOGI("key up forward: %d", forward);
	}
	if (gamekeydown[key_down])
	{
		forward -= forwardmove[pClass][speed];
		//LOGI("key down forward: %d", forward);
	}
	if (joyymove < 0)
	{
		forward += forwardmove[pClass][speed];
...
|| joybuttons[joybstrafe];
	...
	//LOGI("f result: %d, %d", forward, cmd->forwardmove);
	cmd->sidemove += side;
